...
const finalCuerpoTabla = (datos) => {
        ...
        const valorCuota = getValorCuota(diferencias, tasaDiaria, prestamo, fechasCuotas.length, vectores);
        let montoCuota = [];
        ...
        for(let i = 0; i < fechasCuotas.length; i++){
            if(vectores[i]===0){
                principalFinal[i] = principalInicio[i] * Math.pow(1 + tasaDiaria, diferencias[0][i]);
                interesCuota[i]= 0;
                capital[i]=0;
                principalInicio[i+1]=principalFinal[i];
            } else {
                interesCuota[i] = principalInicio[i]* Math.pow(1+tasaDiaria, diferencias[0][i]) - principalInicio[i];
